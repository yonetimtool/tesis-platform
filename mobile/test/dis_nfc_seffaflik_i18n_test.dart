...
void main() {
  ...
  testWidgets('DAR 320dp: dis hizmet ekrani 6 dilde TASMAZ', (tester) async {
    await darEkranSurusu(tester, (dil) => _disEkrani(Locale(dil)),
        veri: surusVerisi);
  });
  testWidgets('DAR 320dp: seffaflik ekrani 6 dilde TASMAZ', (tester) async {
    await darEkranSurusu(tester, (dil) => _seffaflikEkrani(Locale(dil)),
        veri: surusVerisi);
  });

  // ---- TUR 27: YAZI OLCEGI SURUSU (2.0x x 6 dil) ----
  testWidgets('OLCEK 2x: dis ekrani 6 dilde TASMAZ', (tester) async {
    await yaziOlcegiSurusu(tester, (dil) => _disEkrani(Locale(dil)), veri: surusVerisi);
  });
  testWidgets('OLCEK 2x: seffaflik ekrani 6 dilde TASMAZ', (tester) async {
    await yaziOlcegiSurusu(tester, (dil) => _seffaflikEkrani(Locale(dil)), veri: surusVerisi);
  });

  // ---- TUR 29: EKRAN OKUYUCU SURUSU ----
  testWidgets('OKUYUCU: dis ekrani (etiket + dokunma hedefi + dil)',
      (tester) async {
    await ekranOkuyucuSurusu(tester, (dil) => _disEkrani(Locale(dil)),
        veri: surusVerisi);
  });
  testWidgets('OKUYUCU: seffaflik ekrani (etiket + dokunma hedefi + dil)',
      (tester) async {
  ...
  // ---- TUR 33: KLAVYE ----
  testWidgets('KLAVYE: disEkrani (odak sirasi + tuzak + dokunma-yalniz)',
      (tester) async {
    await klavyeSurusu(tester, (dil) => _disEkrani(Locale(dil)));
  });
  testWidgets('KLAVYE: seffaflikEkrani (odak sirasi + tuzak + dokunma-yalniz)',
      (tester) async {
    await klavyeSurusu(tester, (dil) => _seffaflikEkrani(Locale(dil)));
  });
}
