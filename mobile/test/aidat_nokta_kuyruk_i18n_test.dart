...
void main() {
  ...
  testWidgets('OLCEK 2x: aidat ekrani 6 dilde TASMAZ', (tester) async {
    await yaziOlcegiSurusu(tester, (dil) => _aidatEkrani(Locale(dil)), veri: surusVerisi);
  });
  ...
  // ---- TUR 32: KOYU TEMA ----
  testWidgets('KOYU TEMA: aidatEkrani 7 dilde (kontrast + tasma)',
      (tester) async {
    await koyuTemaSurusu(tester, (dil) => _aidatEkrani(Locale(dil)),
        veri: surusVerisi);
  });
  testWidgets('KOYU TEMA: noktaEkrani 7 dilde (kontrast + tasma)',
      (tester) async {
    await koyuTemaSurusu(tester, (dil) => _noktaEkrani(Locale(dil)),
        veri: surusVerisi);
  });

  // ---- TUR 33: KLAVYE ----
  testWidgets('KLAVYE: aidatEkrani (odak sirasi + tuzak + dokunma-yalniz)',
      (tester) async {
    await klavyeSurusu(tester, (dil) => _aidatEkrani(Locale(dil)));
  });
  testWidgets('KLAVYE: noktaEkrani (odak sirasi + tuzak + dokunma-yalniz)',
      (tester) async {
    await klavyeSurusu(tester, (dil) => _noktaEkrani(Locale(dil)));
  });
}
