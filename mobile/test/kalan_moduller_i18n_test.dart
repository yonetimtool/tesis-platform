...
void main() {
  ...
  testWidgets('OKUYUCU: destek ekrani (etiket + dokunma hedefi + dil)',
      (tester) async {
    await ekranOkuyucuSurusu(tester, (dil) => _destekEkrani(Locale(dil)),
        veri: surusVerisi);
  });
  testWidgets('OKUYUCU: tesis ekrani (etiket + dokunma hedefi + dil)',
      (tester) async {
    await ekranOkuyucuSurusu(tester, (dil) => _tesisEkrani(Locale(dil)),
        veri: surusVerisi);
  });
  testWidgets('OKUYUCU: vardiya ekrani (etiket + dokunma hedefi + dil)',
      (tester) async {
    await ekranOkuyucuSurusu(tester, (dil) => _vardiyaEkrani(Locale(dil)),
        veri: surusVerisi);
  });
  testWidgets('OKUYUCU: yonetici ekrani (etiket + dokunma hedefi + dil)',
      (tester) async {
    await ekranOkuyucuSurusu(tester, (dil) => _yoneticiEkrani(Locale(dil)),
        veri: surusVerisi);
  });

  // ---- TUR 32: KOYU TEMA ----
  testWidgets('KOYU TEMA: destekEkrani 7 dilde (kontrast + tasma)',
      (tester) async {
    await koyuTemaSurusu(tester, (dil) => _destekEkrani(Locale(dil)),
        veri: surusVerisi);
  });
  testWidgets('KOYU TEMA: tesisEkrani 7 dilde (kontrast + tasma)',
      (tester) async {
    await koyuTemaSurusu(tester, (dil) => _tesisEkrani(Locale(dil)),
        veri: surusVerisi);
  });
  testWidgets('KOYU TEMA: vardiyaEkrani 7 dilde (kontrast + tasma)',
      (tester) async {
    await koyuTemaSurusu(tester, (dil) => _vardiyaEkrani(Locale(dil)),
        veri: surusVerisi);
  });
  testWidgets('KOYU TEMA: yoneticiEkrani 7 dilde (kontrast + tasma)',
      (tester) async {
    await koyuTemaSurusu(tester, (dil) => _yoneticiEkrani(Locale(dil)),
        veri: surusVerisi);
  });

  // ---- TUR 33: KLAVYE ----
  testWidgets('KLAVYE: destekEkrani (odak sirasi + tuzak + dokunma-yalniz)',
      (tester) async {
    await klavyeSurusu(tester, (dil) => _destekEkrani(Locale(dil)));
  });
  testWidgets('KLAVYE: tesisEkrani (odak sirasi + tuzak + dokunma-yalniz)',
      (tester) async {
    await klavyeSurusu(tester, (dil) => _tesisEkrani(Locale(dil)));
  });
  testWidgets('KLAVYE: vardiyaEkrani (odak sirasi + tuzak + dokunma-yalniz)',
      (tester) async {
    await klavyeSurusu(tester, (dil) => _vardiyaEkrani(Locale(dil)));
  });
  testWidgets('KLAVYE: yoneticiEkrani (odak sirasi + tuzak + dokunma-yalniz)',
      (tester) async {
