...
| 4 | **OAuth geri dönüş adresleri** (Aşama 4) | Google/Microsoft/Apple istemcileri geri dönüş adresini **tam eşleşmeyle** doğrular. Canlı istemciye test adresi eklenirse **canlı istemcinin yapılandırması test için değiştirilmiş** olur. | Her sağlayıcıda **AYRI OAuth istemcisi** açılacak (test istemcisi). **Kaydedilecek adres TEK TANEDİR** — mobil için ayrı kayıt gerekmez: `https://api.test.yonetiyor.com/auth/oauth/callback/<saglayici>`. Sağlayıcı yalnız bu adresi görür; panele ve mobil uygulamaya dönüşü arka uç yapar. Tam konsol adımları: **`docs/oauth-kurulum.md`**. |
