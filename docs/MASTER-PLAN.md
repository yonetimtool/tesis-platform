...
Notes (2026-08-01):
**ÖLÇÜM (22 dk 48 sn):** `1137 passed, 1 skipped, 2 warnings, **1 error**`.
Yani P41'den beri raporlarda tekrarladığım **"1138 passed" bayat bir sayıydı**
ve üstelik suite **tamamen yeşil değil**: `test_unit_complaties.py::
test_p24_okunmamis_diger_suzgeclerle_BIRLIKTE_calisir` **ERROR** veriyor.

**SIRA BAĞIMLI.** Aynı test **tek başına geçiyor** (1 passed, 9 sn) ve
...
**AÇIK İŞ:** ERROR'un kök nedeni (hangi dosya `ucworld`u kirletiyor) henüz
bulunmadı; traceback ikinci tam koşumun logunda olacak
(`scratchpad/pytest.log`). Bir sonraki turun ilk işi budur.
