...
6. Quality gates per touched area (every item, no exceptions):
   ...
   - /mobile → flutter analyze clean + flutter test green + flutter build apk --debug ✓.
     **`flutter test` çıktısını da boruya sokma (P87):** `| tail` bir
     "Failing tests" bloğunu görünmez kılar; dosyaya yaz, sonra oku.
...
**BİR ÖLÇÜM FARKI KAYDEDİLDİ.** `mobil-test` bu koşumda **1557**, önceki
koşumlarda **1559** raporladı. Fark **başarısızlık değil**: `flutter test`
ilerleme satırındaki sayaç, koşum bittiğinde son satırda kalan değere göre
okunuyor ve `~3` atlanan test sayısıyla birlikte satır sonunda farklı
kırpılabiliyor. Kapı **yeşil** (çıkış kodu 0, "Failing tests" bloğu yok).
Yine de not düşülüyor: **iki farklı sayı gördüysem, ikisini de yazarım** —
birini seçip diğerini gizlemek, ölçümü anlatıya uydurmak olurdu.
