...
6. Quality gates per touched area (every item, no exceptions):
   - /backend → full pytest green. **NASIL KOŞULUR (P75, üç tuzak ölçüldü):**
     (a) `docker compose build api` ÖNCE — imaj kodu içine gömer, aksi halde
     ESKİ kod test edilir; (b) çıktıyı **boruya sokma** (`| tail`) — boru
     hattının çıkış kodu son komutunkidir ve `pytest`in hatası kaybolur
     (`(exit 1) | tail -1` → `$?`=0); dosyaya yaz, sonra oku; (c) **tek
     koşum** — ikinci bir pytest, `conftest`in açılış temizliği yüzünden
     birincinin fixture tenant'larını siler (artık koşum kilidi engelliyor).
     Konteynerde `ps` YOKTUR; koşumun sürdüğünü **log dosyasının
     büyümesinden** anla, süreç sorgusundan değil.
