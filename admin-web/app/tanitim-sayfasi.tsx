    ...
    <div dir={yonu} className="min-h-screen bg-white text-ink">
      ...
      <main className="mx-auto max-w-5xl px-5 pb-16">
        ...
        <section className="grid gap-6 py-8 sm:grid-cols-2">
          <article className="rounded-kart border border-[color:var(--yz-border)] bg-[color:var(--yz-bg-app)] p-6">
            <h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">
              {i.yoneticiBaslik}
            </h1>
            <p className="mt-3 text-[color:var(--yz-text)]">{i.yoneticiAlt}</p>
          </article>
          <article className="rounded-kart border border-[color:var(--yz-border)] p-6">
            <h2 className="text-2xl font-semibold tracking-tight sm:text-3xl">
              {i.sakinBaslik}
            </h2>
            <p className="mt-3 text-[color:var(--yz-text)]">{i.sakinAlt}</p>
          </article>
        </section>
        ...
        <section id="ozellikler" className="py-8">
          ...
          <div className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {i.ozellikler.map((o) => (
              <article key={o.baslik} className="rounded-xl border border-[color:var(--yz-border)] p-5">
                <h3 className="font-medium">{o.baslik}</h3>
                <p className="mt-2 text-sm text-[color:var(--yz-text)]">{o.metin}</p>
              </article>
            ))}
          </div>
        </section>
        ...
        <section id="hakkimizda" className="py-8">
          ...
          {i.hakkimizdaParagraflar.map((p) => (
            <p key={p} className="mt-3 max-w-3xl text-[color:var(--yz-text)]">
              {p}
            </p>
          ))}
        </section>
        ...
        <section id="uygulama" className="py-8">
          ...
          <p className="mt-3 max-w-3xl text-[color:var(--yz-text)]">{i.uygulamaYakinda}</p>
        </section>

        <section id="iletisim" className="py-8">
          <h2 className="text-xl font-semibold">{i.iletisimBaslik}</h2>
          <p className="mt-3 max-w-3xl text-[color:var(--yz-text)]">{i.iletisimMetin}</p>
          ...
          <TanitimForm />
          <p className="mt-4 text-satiralt text-[color:var(--yz-text-2)]">
      ...
      <footer className="border-t border-[color:var(--yz-border)]">
        <div className="mx-auto flex max-w-5xl flex-wrap items-center gap-4 px-5 py-6 text-sm text-[color:var(--yz-text)]">
